...
# %%
# setting up logger
logfile = "./log/FEDGE-FS.log"
make_file_dir(logfile)
logger = logging.getLogger("FEDGE-FS logger")
...
def get_parameters():
    ...
    parser.add_argument("--output", type=str,
                        default='output/FEDGE-FS_output.csv', help="output file")
...
model_save_path = "model/FEDGE-FS"
if not os.path.exists(model_save_path):
    os.makedirs(model_save_path)
device = "cuda" if torch.cuda.is_available() else "cpu"
writer = SummaryWriter("runs/fedge-fs_{}".format(target_app))
...
fs = FeatureSelector(app_dim, fs_sigma, device).to(device)

fs_optimizer = optim.Adam(fs.parameters(), lr=ae_lr)
...
def train(model, itr):
    ...
    new_data = fs(app_data)
    code = model.Q(new_data)
    ...
    # Feature selector regularization loss backword
    fs_reg.backward()
    fs_optimizer.step()
    logger.info("\t fs reg loss: {:.4f}".format(fs_reg.item()))
...
with open(os.path.join(model_save_path, "{}_mu.pt".format(target_app)), "wb") as f:
    pickle.dump(fs.get_gates(), f)
