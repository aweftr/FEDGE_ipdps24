# FEDGE method without domain generalization method and STG (multi-domain MMD and adversarial learning are not used)
# %%
import os
import argparse
import logging
...
def get_parameters():
    ...
    parser.add_argument("--output", type=str,
                        default='output/FEDGE-noDG_output.csv', help="output file")
...
model_save_path = "model/FEDGE-noDG"
if not os.path.exists(model_save_path):
    os.makedirs(model_save_path)
device = "cuda" if torch.cuda.is_available() else "cpu"
writer = SummaryWriter("runs/fedge-nodg_{}".format(target_app))
...
def train(model, itr):
    ...
    total_loss = L_qos + lambda_0 * L_re
    logger.info("Iteration {}, L_qos: {:.3f}, L_re: {:.3f}".format(itr, L_qos.item(), L_re.item()))
    model.Q.zero_grad()
    model.P.zero_grad()
    model.R.zero_grad()
    total_loss.backward()
    ...
    P_optimizer.step()
    Q_optimizer.step()
    writer.add_scalar("L_qos", L_qos.item(), itr)
    writer.add_scalar("L_re", L_re.item(), itr)
