# FEDGE without multi-domain MMD loss and STG. 
# %%
import os
import argparse
import logging
...
def get_parameters():
    ...
    parser.add_argument("--output", type=str,
                        default='output/FEDGE-noM_output.csv', help="output file")
...
model_save_path = "model/FEDGE-noM"
if not os.path.exists(model_save_path):
    os.makedirs(model_save_path)
device = "cuda" if torch.cuda.is_available() else "cpu"
writer = SummaryWriter("runs/fedge-nom_{}".format(target_app))
...
def train(model, itr):
    ...
    logger.info("Iteration {}, L_qos: {:.3f}, L_re: {:.3f}, L_adv_min: {:.3f}".format(itr, L_qos.item(), L_re.item(), L_adv_min.item()))
    model.Q.zero_grad()
    model.P.zero_grad()
    model.R.zero_grad()
    total_loss.backward()
