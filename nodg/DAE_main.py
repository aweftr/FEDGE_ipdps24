...
from model import AE, DAE_dataset
...
# %%
def get_parameters():
    parser = argparse.ArgumentParser(description="Denoising Autoencoder for PMc")
    parser.add_argument("--data_dir", type=str,
                        default='data/output/DAE/cassandra-stress_nostress_workload_merged.csv', help="data directory")
    parser.add_argument("--config", type=str,
                        default="config/DAE_config.json", help="config filename")
...
class DAE_trainer():
    def __init__(self, args):
        ...
        self.dataset = DAE_dataset(args.data_dir)
    ...
    def _save_model_state_dict(self):
        app_qos = self.args.data_dir.split("/")[-1].split("-")[0]
        save_filename = self.args.model_savepath + app_qos + "_DAE.pt"
        make_file_dir(save_filename)
        torch.save(self.model.state_dict(), save_filename)

    def _save_finalresult(self, test_loss):
        app_qos = self.args.data_dir.split("/")[-1].split("-")[0]
        savefile = "output/DAE_output.csv"
